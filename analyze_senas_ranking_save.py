...
def display_team_evolution(data: Dict, team_name: str) -> None:
    ...
    for snapshot in evolution[team_id]:
        ...
        if opponent:
            ...
            stats = (
                f"{last_result:2}  {snapshot['date']}  "
                f"{snapshot['position']:3}  {snapshot['points']:3}  "
                f"{snapshot['wins']:3}  {snapshot['losses']:3}  "
                f"{snapshot['points_scored']:5}  {snapshot['points_conceded']:5}  "
                f"{snapshot['point_average']:5}  {score_str:>7}  {match_str}"
            )
            print(stats)
