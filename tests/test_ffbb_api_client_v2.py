...
class TestFFBBAPIClientV2(unittest.TestCase):
    def setUp(self):
        self.meilisearch_client: MeilisearchFFBBAPPClient = MeilisearchFFBBAPPClient(
            meilisearch_ffbb_app_token,
            debug=True,
        )
        self.meilisearch_client_helper: MeilisearchFFBBAPPClientHelper = (
            MeilisearchFFBBAPPClientHelper(self.meilisearch_client)
        )
